...
/* read in triangles. . . */
for (i=0; i<numtris; i++) {
 fscanf(dfile,"%d%d%d%d",&tlist[i].v1,&tlist[i].v2,&tlist[i].v3,&tlist[i].type);
}
...
if ((zt==1)) {
if (tlist[order[i]].type>64) {
   triangle2(scrbuf,x1,y1,x2,y2,x3,y3,tlist[order[i]].type); }
else triangle2(scrbuf,x1,y1,x2,y2,x3,y3,tlist[order[i]].color);
}
